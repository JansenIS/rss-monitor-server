...
- обязательное включение конкретной страны в промпт изображения, запрет стереотипной природы (носорогов, попугаев, крокодилов, джунглей и похожих generic wildlife-сцен), а также запрет национальных/этнических стереотипов вроде клишированной традиционной одежды и generic folk clothing;
...
Каждая публикация получает `scheduled_for`, а при отправке в WordPress API дата передаётся в поле `date`, чтобы пост был опубликован/создан с исторической датой. В ретроспективном режиме все публикации одного календарного дня получают одну и ту же ретроспективную дату `00:00:00Z`; `articles_per_day` задаёт количество публикаций на эту дату для каждого сайта, а не распределение по разным часам внутри дня.
...
Перед каждым вызовом RouterAI worker проверяет эти лимиты по истории `published_articles` для обычного (`recent`) режима. Ретроспективный режим лимиты сайта не учитывает, потому что он назначает архивные даты публикации и должен заполнить выбранный период без пауз по текущему лимиту сайта. Если лимит исчерпан в обычном режиме, статья не генерируется, job переводится в `rate_limited`, получает `retry_after` и будет автоматически продолжена после сброса соответствующего окна. В админке можно сбросить лимит отдельного сайта или всех сайтов; после сброса учитываются только попытки, созданные позже момента сброса.

Задания в админке можно отменять и перезапускать: отмена останавливает дальнейшую обработку очереди задания, а перезапуск возвращает задание в `queued` и переиспользует уже сохранённый snapshot, пропуская ранее опубликованные слоты.
