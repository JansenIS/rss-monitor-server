...
При открытии `http://SERVER_IP:5173` поле `Server URL` автоматически подставит API на том же хосте: `http://SERVER_IP:8080`. Проверь Bearer token из `.env` и нажми `Проверить API`.
